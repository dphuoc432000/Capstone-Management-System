...
class NotificationService {
    ...
    addNotification = async (notification, files) => {
        ...
        if (files !== undefined || files.length > 0) {
            console.log("vao");
            fileStorageDataArray = await fileStorageService.addFiles(files, notification.userId);
        }
    ...
    //    file4: {}/null
    //  }
    updateNotificationByNotificationId = async (notificationId, newNotification) => {
        ...
        return await Notification.findOne({
            ...
            .then(async notificationData => {
                ...
                if (notificationData && checkObject(notificationData)) {
                    ...
                    const notificationUpdate = notificationData.get({ plain: true });
                    ...
                    for (const [key, value] of Object.entries(oldFiles)) {
                        //nếu fileId1 của notification có,
                        //Nếu  fileId1 của notification = fileId ủa file1 cũ
                        if (notificationData.fileId1 && notificationData.fileId1 === value.fileId) {
                        ...
                        else if (notificationData.fileId2 && notificationData.fileId2 === value.fileId) {
                            notificationUpdate.fileId2 = null;
                            fileIdNeedDelete.push(value.fileId)
                        }
                        else if (notificationData.fileId3 && notificationData.fileId3 === value.fileId) {
                            notificationUpdate.fileId3 = null;
                            fileIdNeedDelete.push(value.fileId)
                        }
                        else if (notificationData.fileId4 && notificationData.fileId4 === value.fileId) {
                    ...
                    const filesDataArr = await fileStorageService.addFiles(newFiles);
                    ...
                    const rowUpdate = await Notification.update(notificationUpdate, {
                        where: {
                            notificationId
                        }
                    })
                    ...
                    if (fileIdNeedDelete.length > 0) {
                        ...
                        if (fileIdNeedDelete.length > 0) {
                            countRowDelete = await fileStorageService.deleteFilesByFileIds(fileIdNeedDelete);
                        }
                        // console.log("countRowDelete: ", countRowDelete);
                    }
                    if (countRowUpdate > 0)
                        return true;
                }
    ...
    deleteNotificationByNotificationID = async (notificationId) => {
        return await Notification.findOne({
            ...
            .then(async data => {
                if (data) {
                    // console.log(data);
                    const fileIds = [data.fileId1, data.fileId2, data.fileId3, data.fileId4].filter(file => file && file);
                    if (fileIds.length > 0) {
                        const countRowFileStorageDelete = await fileStorageService.deleteFilesByFileIds(fileIds)
                        if (countRowFileStorageDelete > 0)
                            return true;
                    }
                    else {
                        const countRowNotificatitonDelete = await Notification.destroy({ where: { notificationId } })
                        if (countRowNotificatitonDelete > 0)
                            return true;
                    }
                }
                return false;
